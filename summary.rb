...
data = []
original_lines=`grep -r -h '^[^[:space:]#]' en/[0]* | grep -v '^Insereix'| wc -l`.to_i
Dir.glob("*").each do |dir|
  if !File.file?(dir) && !exclude.include?(dir)
    lines = `git diff-tree -r -p --diff-filter=M master:en master:#{dir} | grep '^-[^[:space:]#-]' | grep -v '^-Insereix' | wc -l`.strip.to_i
